...
def test_create_user_and_verify_audit_logs(client):
    response = client.post(
        "/users/",
        json={"name": "Audit Test User", "email": "audittestuser@example.com"},
    )
...
def test_user_crud_and_audit(client):
    ...
    assert audit_response_data[0]["changed_fields"] == {
        "new": {"email": "testuser@example.com", "name": "Test User"},
        "old": {},
    }
    ...
    audit_response_json = client.get(f"/audit/?user_id={created_user['id']}").json()[1][
        "changed_fields"
    ]
    assert audit_response_json == {
        "old": {"name": "Test User", "email": "testuser@example.com"},
        "new": {"name": "Updated Test User", "email": "updateduser@example.com"},
    }
